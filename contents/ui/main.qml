...
PlasmoidItem {
            ...
            if (stdout.length > 10) {
                try {
                    ...
                    if (age < 86400000) {
                        ...
                        console.log("Synthetic Usage: Loaded cache, age:", Math.round(age / 60000), "min, stale:", root.isStale)
                    } else {
                        console.log("Synthetic Usage: Cache too old, ignoring")
                    }
                } catch (e) {
                    console.log("Synthetic Usage: Cache parse error:", e)
                }
            }
        }
    }
    ...
    function saveCache() {
        ...
        cacheWriter.connectSource("echo '" + json.replace(/'/g, "'\\''") + "' > $HOME/.local/share/synthetic-usage-cache.json")
    }
    ...
    Plasma5Support.DataSource {
        ...
        onNewData: function(sourceName, data) {
            ...
            try {
                ...
                console.log("Synthetic Usage: JSON parse error:", e, stdout)
            ...
            if (!resp.ok) {
                ...
                console.log("Synthetic Usage: fetch error:", msg)
            ...
            // Weekly token limit — primary metric
            var wtl = d.weeklyTokenLimit || {}
            var pctRem = wtl.percentRemaining || 0
            root.primaryPercent = Math.max(0, 100 - pctRem)
            root.planName = wtl.maxCredits || ""
            root.creditsInfo = (wtl.remainingCredits || "?") + " / " + (wtl.maxCredits || "?")
            if (wtl.nextRegenCredits && wtl.nextRegenCredits !== "$0.00") {
                root.creditsInfo += "  (+" + wtl.nextRegenCredits + " " + i18n("regen") + ")"
            }
            root.primaryResetTime = wtl.nextRegenAt ? new Date(wtl.nextRegenAt) : null

            // Rolling 5-hour limit — secondary metric
            var r5 = d.rollingFiveHourLimit || {}
            var rMax = r5.max || 1
            var rRem = r5.remaining !== undefined ? r5.remaining : rMax
            root.secondaryPercent = Math.max(0, (1 - rRem / rMax) * 100)
            root.secondaryResetTime = r5.nextTickAt ? new Date(r5.nextTickAt) : null

            // Subscription quota
            var sub = d.subscription || {}
            root.subscriptionInfo = (sub.requests !== undefined ? sub.requests : "?") + " / " + (sub.limit || "?") + " " + i18n("requests")

            // Search quota
            var srch = (d.search || {}).hourly || {}
            root.searchInfo = (srch.requests !== undefined ? srch.requests : "?") + " / " + (srch.limit || "?") + " " + i18n("/ hr")
            ...
            console.log("Synthetic Usage: fetch success - weekly:", root.primaryPercent, "rolling:", root.secondaryPercent)
        }
    }
    ...
    compactRepresentation: Item {
        ...
        RowLayout {
            ...
            Item {
                ...
                Kirigami.Icon {
                    anchors.fill: parent
                    source: Qt.resolvedUrl("../icons/synthetic-usage-widget.svg")
                }
        ...
        ColumnLayout {
            ...
            Rectangle {
                ...
                ColumnLayout {
                    ...
                    PlasmaComponents.Label {
                        text: i18n("Add a synthetic.new key to ~/.config/synthetic/api-key")
                        font.pixelSize: Kirigami.Theme.smallFont.pixelSize
            ...
            ColumnLayout {
                ...
                RowLayout {
                    Layout.fillWidth: true
                    PlasmaComponents.Label {
                        text: i18n("Weekly Token Credits")
                        font.bold: true
                    }
                    Item { Layout.fillWidth: true }
                    PlasmaComponents.Label {
                ...
                PlasmaComponents.Label {
                    ...
                    text: i18n("Regen in") + " " + formatTimeRemaining(root.primaryResetTime)
                    font.pixelSize: Kirigami.Theme.smallFont.pixelSize
                    color: Kirigami.Theme.disabledTextColor
                }
            }
            ...
            ColumnLayout {
                ...
                PlasmaComponents.Label {
                    ...
                    text: i18n("Next tick in") + " " + formatTimeRemaining(root.secondaryResetTime)
                    font.pixelSize: Kirigami.Theme.smallFont.pixelSize
                    color: Kirigami.Theme.disabledTextColor
                }
            }
    ...
    Component.onCompleted: {
        console.log("Synthetic Usage: Widget loaded")
        var iconSource = Qt.resolvedUrl("../icons/synthetic-usage-widget.svg").toString().replace("file://", "")
        iconInstaller.connectSource("bash -c 'ICON_DIR=${XDG_DATA_HOME:-$HOME/.local/share}/icons/hicolor/scalable/apps && mkdir -p $ICON_DIR && cp \"" + iconSource + "\" $ICON_DIR/synthetic-usage-widget.svg && chmod 644 $ICON_DIR/synthetic-usage-widget.svg 2>/dev/null'")
        cacheReader.connectSource("cat $HOME/.local/share/synthetic-usage-cache.json 2>/dev/null")
        fetchUsage()
    }
    ...
    Plasmoid.icon: "synthetic-usage-widget"
    toolTipMainText: i18n("Synthetic Usage")
    toolTipSubText: {
        var parts = []
